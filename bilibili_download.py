...
def get_bilibili_download_path():
    downloads = os.path.join(os.path.expanduser("~"), "Downloads")
    bili_folder = os.path.join(downloads, "Downloads From Bilibili")
    os.makedirs(bili_folder, exist_ok=True)
    return bili_folder

class BiliDlpWorker(QtCore.QThread):
    ...
    def run(self):
        ydl_opts = {
            ...
            'quiet': True,
            'noplaylist': False,  # Bilibili videos can be multi-part, will auto-merge.
        }
...
class BilibiliDownloaderUI(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Coke Bilibili Downloader")
        self.resize(800, 600)
        ...
        label = QtWidgets.QLabel("Bilibili Downloader")
        label.setStyleSheet("font-size: 40px; color: #b4aaff; font-weight: bold;")
        label.setAlignment(QtCore.Qt.AlignCenter)
        layout.addWidget(label)

        self.url_input = QtWidgets.QLineEdit(self)
        self.url_input.setPlaceholderText("Paste Bilibili video link here...")
        ...
        self.dl_btn.clicked.connect(self.download_bilibili)
    ...
    def download_bilibili(self):
        ...
        outdir = get_bilibili_download_path()
        # Only allow public bilibili links (no cookies, so no bangumi/paid/private)
        if not url or ("bilibili.com" not in url.lower() and "b23.tv" not in url.lower()):
            self.status.setText("Please enter a valid Bilibili video link.")
            return
        if "bangumi" in url or "play/ep" in url:
            self.status.setText("Bangumi/Anime and member-only videos are not supported.\nPlease use a public Bilibili video link.")
            return
        ...
        self.worker = BiliDlpWorker(url, fmt, outdir)
